...
export function validateClientProfileForm(data: {
  ...
  if (data.weight) {
    const weightValidation = validateNumber(data.weight, "Weight", 80, 350);
    if (!weightValidation.valid)
      errors.weight = weightValidation.error || "Invalid weight";
  }

  if (data.height) {
    const heightValidation = validateNumber(data.height, "Height", 36, 96);
  ...
  if (data.bodyFatPercentage) {
    const bfValidation = validateNumber(
      data.bodyFatPercentage,
      "Body Fat %",
      2,
      60,
    );
    if (!bfValidation.valid)
      errors.bodyFatPercentage = bfValidation.error || "Invalid body fat %";
  }

  if (data.musclePercentage) {
    const muscleValidation = validateNumber(
    ...
    if (!muscleValidation.valid)
      errors.musclePercentage = muscleValidation.error || "Invalid muscle %";
  }

  if (data.goalWeight) {
    const goalValidation = validateNumber(
      data.goalWeight,
      "Goal Weight",
      20,
      300,
    ...
    if (!goalValidation.valid)
      errors.goalWeight = goalValidation.error || "Invalid goal weight";
  }

  if (data.goalDescription && data.goalDescription.length > 500) {
    errors.goalDescription =
      "Goal description must be less than 500 characters";
  }
...
export function validateTrainerProfileForm(data: {
  ...
  if (
    data.themeColor &&
    !/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/.test(data.themeColor)
  ) {
...
export function validateTrainerPassword(password: string): {
  valid: boolean;
  error?: string;
} {
  if (!password) return { valid: false, error: "Password is required" };
